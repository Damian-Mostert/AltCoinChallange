...
export default function Home() {
  ...
  return (
    <div className='d-flex w-100 h-100 align-items-center justify-content-center gap-4 p-4'>
        <div className='p-4 border border-accent bg-bg-fg rounded-4 w-100 h-100  overflow-auto'>
          {loadingData ? <div className='w-100 h-100 d-flex align-items-center justify-content-center'><div className='loader'/></div>:<>
              {data ? <pre><code>{JSON.stringify(data,null,4)}</code></pre> : <div className='w-100 h-100 d-flex align-items-center justify-content-center'>
                Could not load data    
            </div>}
          </>}
        </div>
